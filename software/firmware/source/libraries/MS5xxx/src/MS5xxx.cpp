...
unsigned int MS5xxx::Calc_CRC4(unsigned char poly)
{
  ...
  {                              // choose LSB or MSB
    if (cnt % 2 == 1)
      ...
      n_rem ^= (unsigned short)((C[cnt >> 1] >> 8) & 0x00FF);
...
byte MS5xxx::cmd_adc_conversion(unsigned char aCMD)
{
  byte ret = send_cmd(MS5xxx_CMD_ADC_CONV + aCMD);
  if (!ret)
    lastADCReadoutCmd = millis();
  return ret; // start DAQ and conversion of ADC data
}

unsigned long MS5xxx::read_adc_nonblocking()
{
  unsigned long value = 0;
  unsigned long c = 0;
...
//This non-blocking method will read new data every 2 calls, if the calls are at least 11 ms apart.
//Returns true if new data is available.
bool MS5xxx::ReadoutNonblocking()
{
  bool ret = 0;
  switch (status)
  {
  case STATUS::CONVERTING_D1:
  {
    // Serial.println(F("status = STATUS::CONVERTING_D1;"));
    if (millis() >= lastADCReadoutCmd + 11)
    {
      D1 = read_adc_nonblocking();

      if (!cmd_adc_conversion(MS5xxx_CMD_ADC_D2 + MS5xxx_CMD_ADC_4096))
        status = STATUS::CONVERTING_D2;
      
    }
    break;
  }
  case STATUS::CONVERTING_D2:
  {
    // Serial.println(F("status = STATUS::CONVERTING_D2;"));
    if (millis() >= lastADCReadoutCmd + 11)
    {
      D2 = read_adc_nonblocking();
      status = STATUS::READY;
    }
    //fallthrough
  }
  ...
  default:
  {
    // Serial.println(F("status = STATUS::STANDBY;"));
    if (!cmd_adc_conversion(MS5xxx_CMD_ADC_D1 + MS5xxx_CMD_ADC_4096))
      status = STATUS::CONVERTING_D1;
